...
public class DatabaseIntrospector {
    ...
    private void calculateIndex(FullyQualifiedTable table,
                                     IntrospectedTable introspectedTable) {
        ResultSet rs = null;
        ...
        try {
            rs = databaseMetaData.getIndexInfo(
                    table.getIntrospectedCatalog(), table
                            .getIntrospectedSchema(), table
                            .getIntrospectedTableName(),false,false);
        } catch (SQLException e) {
            closeResultSet(rs);
            warnings.add(getString("Warning.15")); //$NON-NLS-1$
            return;
        ...
        try {
            ...
            while (rs.next()) {
                String columnName = rs.getString("COLUMN_NAME"); //$NON-NLS-1$
                String index_name = rs.getString("INDEX_NAME"); //$NON-NLS-1$
                introspectedTable.addIndexColumn(index_name,columnName);
            }
